...
const companySeries = allSeries.companySeries.map((x) => Object.assign(x, {type: 'Company'}))
const exerciseSeries = allSeries.exerciseSeries.map((x) => Object.assign(x, {type: 'Exercise'}))
...
process.stdin
    ...
    .on('end', () => {
        ...
        companySeries.concat(exerciseSeries).forEach((series) => {
            ...
            db.ref('/teams')
                ...
                .once('value', (snapshot) => {
                    ...
                    matches
                        ...
                        .forEach((match) => {
                            const homeTeam = teams.find((team) => team.teamRanking === match.home_team)
                            const awayTeam = teams.find((team) => team.teamRanking === match.away_team)
                            ...
                            console.log(
                                migratedMatch.homeTeam.teamName +
                                    ' - ' +
                                    migratedMatch.awayTeam.teamName +
                                    ', ' +
                                    date +
                                    ' kl ' +
                                    time
